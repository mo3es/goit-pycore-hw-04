from constants import pattern_all, \
    pattern_change, pattern_hello, pattern_quit,\
    pattern_show, pattern_add, INCORRECT_MESSAGE, USAGE
from contact_handler import add_contact, change_contact,\
    show_all, show_contact
...
def input_parser(user_input: str) -> list():
    ...
    cmd, *args = user_input.strip().split('\w')
    cmd = cmd.strip().lower()
    return cmd, *args
